...
if (nullOrString)
    nullOrString; // let nullOrString: string[] -> nullOrString 값이 true 이기 때문
else
    nullOrString; // let nullOrString: null
// 4) Equality Narrowing
let numberOrString2 = Math.random() > 0.5 ? 1123 : '아이유';
let stringOrBool2 = Math.random() > 0.5 ? '아이브' : true;
...
let numberOrStringOrNull = Math.random() > 0.5 ? 1123 : Math.random() > 0.5 ? '안유진' : null;
if (typeof numberOrStringOrNull === 'number')
    numberOrStringOrNull; // => number
else
    numberOrStringOrNull; // -> null | string
let human = {
    name: '안유진',
    age: 23,
};
...
let humanOrDog = Math.random() > 0.5 ? human : dog;
...
// 6) instanceof Narrowing
let dateOrString = Math.random() > 0.5 ? new Date() : '김영훈';
...
switch (humanOrDog2.type) {
    ...
    default:
        ...
        const _check = humanOrDog2; // -> 이런 값을 추가하여 , 추후 HumanOrDog 타입이 확장될 경우, 오류를 발생시켜 코드를 수정할 수 있도록 한다.
        break;
}
